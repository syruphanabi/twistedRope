...
//----------------- 
void strip(pt P, vec V, vec I1, vec J1, vec I2, vec J2, float r, float rd, float t1, float t2) {
  float da = TWO_PI/pipeRenderLevel;
  beginShape(QUAD_STRIP);
    for(float a=t1*TWO_PI; a<=t2*TWO_PI+da; a+=da) {v(P(P,r*cos(a),I1,r*sin(a),J1,0,V)); v(P(P,rd*cos(a),I2,rd*sin(a),J2,1,V));}
  endShape();
  }
  
void strip2(pt P, vec V, vec I1, vec J1, vec I2, vec J2, float r, float rd) {
  float da = TWO_PI/pipeRenderLevel;
  beginShape(QUAD_STRIP);
    for(float a=0; a<=TWO_PI+da; a+=da) {v(P(P,r*cos(a),I1,r*sin(a),J1,0,V)); v(P(P,rd*cos(a),I2,rd*sin(a),J2,1,V));}
  endShape(); 
  }
  
void stub_strip(pt P, vec V, vec I1, vec J1, vec I2, vec J2, float r, float rd, float init, float tw) // cone section
  {
    fill(yellow); strip(P,V,I1,J1,I2,J2,r,rd, init+tw+0, init+tw+0.25); 
    fill(orange); strip(P,V,I1,J1,I2,J2,r,rd, init+tw+0.25, init+tw+0.5);
    fill(darkWood); strip(P,V,I1,J1,I2,J2,r,rd, init+tw+0.5, init+tw+0.75);
    fill(pink); strip(P,V,I1,J1,I2,J2,r,rd, init+tw+0.75, init+tw+1);
  }
  
void stub_strip2(pt P, vec V, vec I1, vec J1, vec I2, vec J2, float r, float rd, float init, float tw) // cone section
  {
    fill(green); strip(P,V,I1,J1,I2,J2,r,rd, init+tw+0, init+tw+0.33); 
    fill(dgreen); strip(P,V,I1,J1,I2,J2,r,rd, init+tw+0.33, init+tw+0.67);
    fill(lime);strip(P,V,I1,J1,I2,J2,r,rd, init+tw+0.67, init+tw+1);
  }
//---------------------
