...
//  if(key=='`') picking=true; 
  ...
  if(key=='6') {showMainPipe = true; editMode = false; resultMode = false; preTwistMode = false; print(6);}
  if(key=='7') {showMainPipe = false; editMode = true; resultMode = false; preTwistMode = false; print(7);}
  if(key=='8') {showMainPipe = false; editMode = false; resultMode = false; preTwistMode = true; print(8);}
  if(key=='9') {showMainPipe = false; editMode = false; resultMode = true; preTwistMode = false; print(9);}
   
  if(key=='0') braidsMode = 0;
  if(key=='1') braidsMode = 1;
  if(key=='2') braidsMode = 2;
  
  if(key=='b') ctrlVecMode = !ctrlVecMode;
  
  if(key=='f') twist = global_fixed_twist;
  //if(key=='b') showMainPipe = !showMainPipe;
...
//  if (!keyPressed) {Of.add(ToIJ(V((float)(mouseX-pmouseX),(float)(mouseY-pmouseY),0))); }
  ...
  if (keyPressed && key=='s') {
    twist += (float)(mouseX-pmouseX)/50;
    if (twist > 1) twist = 1;
    if (twist < 0) twist = 0;
  }
  if (keyPressed && key=='u') {
    upath += (float)(mouseX-pmouseX)/50;
    if (upath > 1) upath = 1;
    if (upath < 0) upath = 0;
  }
  change=true;
